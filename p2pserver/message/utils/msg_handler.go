...
import (
	...
	actor "github.com/polynetwork/poly/p2pserver/actor/req"
	msgCommon "github.com/polynetwork/poly/p2pserver/common"
	"github.com/polynetwork/poly/p2pserver/message/msg_pack"
	msgTypes "github.com/polynetwork/poly/p2pserver/message/types"
...
func BlkHeaderHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	if pid != nil {
		...
		for _, header := range blkHeader.BlkHdr {
			log.Debugf("[p2p] received block header, addr %s, id %v, header %x, height %v hash %s", data.Addr, data.Id, header.ToArray(), header.Height, header.Hash())
		}
...
func BlockHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	if pid != nil {
		...
		log.Debugf("[p2p] received block, addr %s, id %v, header %x, root %s, height %v hash %s", data.Addr, data.Id,
			block.Blk.Header.ToArray(), block.MerkleRoot.ToHexString(), block.Blk.Header.Height, block.Blk.Hash())
...
func ConsensusHandle(data *msgTypes.MsgPayload, p2p p2p.P2P, pid *evtActor.PID, args ...interface{}) {
	...
	if actor.ConsensusPid != nil {
		...
		bytes, err := json.Marshal(consensus.Cons)
